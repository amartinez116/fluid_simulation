    ...
    for (int j = 0; j < mParticles.size(); j++) {
        ...
        sum += usePressureKernel_gradient(position - mParticles[j].mPosition, SUPPORT_RADIUS) *
               (pressure / (density * density) +
                mParticles[j].mPressure / (mParticles[j].mDensity * mParticles[j].mDensity)) * mParticles[j].mMass;
    }
    return -(sum * density);
}
...
Vector3f Fluid::calcViscosityForce(int indexOfCurrentParticle, Vector3f velocity, Vector3f position) {
    ...
    for (int j = 0; j < mParticles.size(); j++) {
        ...
        sum += (mParticles[j].mVelocity - velocity) * (mParticles[j].mMass / mParticles[j].mDensity) *
               useViscosityKernel_laplacian(position - mParticles[j].mPosition, SUPPORT_RADIUS);
    }
...
bool Fluid::detectCollision(Particle particle, Vector3f &contactPoint, Vector3f &unitSurfaceNormal) {
    if (abs(particle.mPosition.x) <= BOX_SIZE / 2 && abs(particle.mPosition.y) <= BOX_SIZE / 2 &&
...
void Fluid::updateVelocity(Vector3f &velocity, Vector3f unitSurfaceNormal, float penetrationDepth) {
    velocity = velocity - unitSurfaceNormal * (1 + RESTITUTION * penetrationDepth / (TIME_STEP * velocity.length())) *
...
float Fluid::useDefaultKernel_laplacian(Vector3f distVector, float supportRadius) {
    ...
    if (dist > supportRadius)
        ...
        return -(945 / (32 * M_PI * powf(supportRadius, 9.0f))) * (supportRadius * supportRadius - dist * dist) *
...
Vector3f Fluid::usePressureKernel_gradient(Vector3f distVector, float supportRadius) {
    ...
    if (dist > supportRadius)
        ...
        return -(Vector3f(1.0f, 1.0f, 1.0f).normalize() * (45 / (M_PI * powf(supportRadius, 6.0f))) *

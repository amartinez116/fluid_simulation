...
Fluid::Fluid(void) {
  ...
  for (double x = -particleRadius * SIZE; x <= particleRadius * SIZE;
       x += particleDiameter) {
    for (double y = -particleRadius * SIZE; y <= particleRadius * SIZE;
         y += particleDiameter) {
      for (double z = -particleRadius * SIZE; z <= particleRadius * SIZE;
           z += particleDiameter) {
...
void Fluid::draw(void) {
  float sphereRadius =
      powf((3 * MASS) / (4 * M_PI * REST_DENSITY), 1.0f / 3.0f);
  for (int i = 0; i < mParticles.size(); i++) {
    glPushMatrix();
    glColor3f(1.0f, 1.0f, 1.0f);
    glTranslatef(mParticles[i].mPosition.x, mParticles[i].mPosition.y,
                 mParticles[i].mPosition.z);
...
void Fluid::simulate_cuda(void) {
  ...
  cudaComputeDensities(cudaParticles, mParticles.size());
  cudaComputeInternalForces(cudaParticles, mParticles.size());
  cudaComputeExternalForces(cudaParticles, mParticles.size());
  cudaCollisionHandling(cudaParticles, mParticles.size());
...
float Fluid::calcDensity(Vector3f position) {
  ...
  for (int j = 0; j < mParticles.size(); j++)
    sum += mParticles[j].mMass *
           useDefaultKernel(position - mParticles[j].mPosition, SUPPORT_RADIUS);
  return sum;
}
...
Vector3f Fluid::calcPressureForce(int indexOfCurrentParticle, float density,
                                  float pressure, Vector3f position) {
  Vector3f sum(0.0f, 0.0f, 0.0f);
  for (int j = 0; j < mParticles.size(); j++) {
    if (j == indexOfCurrentParticle)
      continue;
    sum += usePressureKernel_gradient(position - mParticles[j].mPosition,
                                      SUPPORT_RADIUS) *
           (pressure / (density * density) +
            mParticles[j].mPressure /
                (mParticles[j].mDensity * mParticles[j].mDensity)) *
           mParticles[j].mMass;
  }
  return -(sum * density);
}

Vector3f Fluid::calcViscosityForce(int indexOfCurrentParticle,
                                   Vector3f velocity, Vector3f position) {
  Vector3f sum(0.0f, 0.0f, 0.0f);
  for (int j = 0; j < mParticles.size(); j++) {
    if (j == indexOfCurrentParticle)
      continue;
    sum += (mParticles[j].mVelocity - velocity) *
           (mParticles[j].mMass / mParticles[j].mDensity) *
           useViscosityKernel_laplacian(position - mParticles[j].mPosition,
                                        SUPPORT_RADIUS);
  }
...
Vector3f Fluid::calcSurfaceNormal(Vector3f position) {
  Vector3f sum(0.0f, 0.0f, 0.0f);
  for (int j = 0; j < mParticles.size(); j++)
    sum += useDefaultKernel_gradient(position - mParticles[j].mPosition,
                                     SUPPORT_RADIUS) *
           (mParticles[j].mMass / mParticles[j].mDensity);
  return sum;
}

Vector3f Fluid::calcSurfaceTensionForce(Vector3f surfaceNormal,
                                        Vector3f position) {
  float sum = 0.0f;
  for (int j = 0; j < mParticles.size(); j++)
    sum += (mParticles[j].mMass / mParticles[j].mDensity) *
           useDefaultKernel_laplacian(position - mParticles[j].mPosition,
                                      SUPPORT_RADIUS);
...
bool Fluid::detectCollision(Particle particle, Vector3f &contactPoint,
                            Vector3f &unitSurfaceNormal) {
  if (abs(particle.mPosition.x) <= BOX_SIZE / 2 &&
      abs(particle.mPosition.y) <= BOX_SIZE / 2 &&
      abs(particle.mPosition.z) <= BOX_SIZE / 2)
    return false;
...
void Fluid::updateVelocity(Vector3f &velocity, Vector3f unitSurfaceNormal,
                           float penetrationDepth) {
  velocity = velocity - unitSurfaceNormal *
                            (1 + RESTITUTION * penetrationDepth /
                                     (TIME_STEP * velocity.length())) *
                            velocity.dot(unitSurfaceNormal);
}
...
float Fluid::useDefaultKernel(Vector3f distVector, float supportRadius) {
  ...
  if (dist > supportRadius)
    ...
    return (315 / (64 * M_PI * powf(supportRadius, 9.0f))) *
           powf(supportRadius * supportRadius - dist * dist, 3.0f);
}

Vector3f Fluid::useDefaultKernel_gradient(Vector3f distVector,
                                          float supportRadius) {
  float dist = distVector.length();
  if (dist > supportRadius)
    return Vector3f(0.0f, 0.0f, 0.0f);
  else
    return -(distVector * (945 / (32 * M_PI * powf(supportRadius, 9.0f))) *
             powf(supportRadius * supportRadius - dist * dist, 2.0f));
}

float Fluid::useDefaultKernel_laplacian(Vector3f distVector,
                                        float supportRadius) {
  float dist = distVector.length();
  if (dist > supportRadius)
    return 0.0f;
  else
    return -(945 / (32 * M_PI * powf(supportRadius, 9.0f))) *
           (supportRadius * supportRadius - dist * dist) *
           (3 * supportRadius * supportRadius - 7 * dist * dist);
}

Vector3f Fluid::usePressureKernel_gradient(Vector3f distVector,
                                           float supportRadius) {
  float dist = distVector.length();
  if (dist > supportRadius)
    return Vector3f(0.0f, 0.0f, 0.0f);
  else if (dist < 10e-5) // If ||r|| -> 0+
    return -(Vector3f(1.0f, 1.0f, 1.0f).normalize() *
             (45 / (M_PI * powf(supportRadius, 6.0f))) *
             powf(supportRadius - dist, 2.0f));
  else
    return -(distVector.normalize() *
             (45 / (M_PI * powf(supportRadius, 6.0f))) *
             powf(supportRadius - dist, 2.0f));
}

float Fluid::useViscosityKernel_laplacian(Vector3f distVector,
                                          float supportRadius) {
  float dist = distVector.length();
  if (dist > supportRadius)
    return 0.0f;
